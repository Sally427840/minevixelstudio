...
export default async function handler(req, res) {
  ...
  try {
    var body = req.body;
    var addonType = body.addonType;
    var addonName = body.addonName;
    ...
    var prompt = 'You are a Minecraft Bedrock addon expert. Generate addon data.\n' +
      'Type: ' + addonType + '\n' +
      'Name: ' + addonName + '\n' +
    ...
    var isCompanion = (behavior === 'Companion' || behavior === 'Passive');
    ...
    if (isHostile) {
      entityComponents['minecraft:attack'] = { damage: parsed.attack || 4 };
      entityComponents['minecraft:behavior.nearest_attackable_target'] = {
        priority: 2,
        entity_types: [{ filters: { test: 'is_family', subject: 'other', value: 'player' }, max_dist: 16 }]
      };
      entityComponents['minecraft:behavior.melee_attack'] = { priority: 3 };
    }

    if (isCompanion) {
      entityComponents['minecraft:behavior.follow_owner'] = {
        priority: 1, speed_multiplier: 1.2, start_distance: 10, stop_distance: 2
      };
    }

    var files = {};

    files['resource_pack/manifest.json'] = JSON.stringify({
      format_version: 2,
      header: { name: addonName + ' Resources', description: description, uuid: uuid1, version: [1,0,0], min_engine_version: [1,20,0] },
      modules: [{ type: 'resources', uuid: uuid2, version: [1,0,0] }]
    }, null, 2);

    files['resource_pack/entity/' + id + '.entity.json'] = JSON.stringify({
    ...
    files['resource_pack/models/entity/' + id + '.geo.json'] = JSON.stringify({
    ...
    files['resource_pack/textures/entity/' + id + '.png'] = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==';

    files['behavior_pack/manifest.json'] = JSON.stringify({
      format_version: 2,
      header: { name: addonName + ' Behaviors', description: description, uuid: uuid3, version: [1,0,0], min_engine_version: [1,20,0] },
      modules: [{ type: 'data', uuid: uuid4, version: [1,0,0] }],
      dependencies: [{ uuid: uuid1, version: [1,0,0] }]
    }, null, 2);

    files['behavior_pack/entities/' + id + '.json'] = JSON.stringify({
